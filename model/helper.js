...
module.exports = async function db(query) {
  ...
  let promise = await new Promise((resolve, reject) => {
    ...
    const con = mysql.createConnection({
      ...
      user: DB_USER || "root",
      password: DB_PASS,
      database: DB_NAME || "makanbah", // changed database name
    ...
    con.connect(function(err) {
      ...
      con.query(query, function(err, result) {
        ...
        console.log("Result:", result); //actual result from database is here. Newly added
        if (!result.length) {
          ...
          results.insertId = result.insertId; // newly added
